...
!
!
! Routines for complex-valiable CG
!
MODULE komega_cg_c
  !
  PRIVATE
...
!
#if defined(MPI)
SUBROUTINE pkomega_CG_C_init(ndim0, nl0, nz0, x, z0, itermax0, threshold0, comm0) BIND(C)
#else
SUBROUTINE komega_CG_C_init(ndim0, nl0, nz0, x, z0, itermax0, threshold0) BIND(C)
#endif
  !
...
!
#if defined(MPI)
SUBROUTINE pkomega_CG_C_update(v12, v2, x, r_l, status) BIND(C)
#else
SUBROUTINE komega_CG_C_update(v12, v2, x, r_l, status) BIND(C)
#endif
  !
  USE komega_parameter, ONLY : iter, itermax, ndim, nl, nz, &
...
!
#if defined(MPI)
SUBROUTINE pkomega_CG_C_getcoef(alpha_save0, beta_save0, z_seed0, r_l_save0) BIND(C)
#else
SUBROUTINE komega_CG_C_getcoef(alpha_save0, beta_save0, z_seed0, r_l_save0) BIND(C)
#endif
  !
...
!
#if defined(MPI)
SUBROUTINE pkomega_CG_C_getvec(r_old) BIND(C)
#else
SUBROUTINE komega_CG_C_getvec(r_old) BIND(C)
#endif
  !
...
!
#if defined(MPI)
SUBROUTINE pkomega_CG_C_getresidual(res) BIND(C)
#else
SUBROUTINE komega_CG_C_getresidual(res) BIND(C)
#endif
  !
...
!
#if defined(MPI)
SUBROUTINE pkomega_CG_C_finalize() BIND(C)
#else
SUBROUTINE komega_CG_C_finalize() BIND(C)
#endif
  !
