...
\noindent Fortran
\begin{verbatim}
CALL CG_R_init(ndim, nl, nz, x, z, itermax, threshold, status)
CALL CG_C_init(ndim, nl, nz, x, z, itermax, threshold, status)
CALL COCG_init(ndim, nl, nz, x, z, itermax, threshold, status)
CALL BiCG_init(ndim, nl, nz, x, z, itermax, threshold, status)
\end{verbatim}
...
\item \verb|ndim, nl, nz, x, z, itermax, threshold, status|
...
\item \verb|status|
  ...
  \item [第一成分(\texttt{status(1)})]
    解が収束した場合には現在の総反復回数が返される.
    \verb|?_init|ルーチンで, \verb|itermax|を有限にして, かつ\verb|itermax|回の反復で
    収束に達しなかった場合には\verb|-itermax|が返される.
    それ以外の場合には\verb|0|が返される.
    
  \item [第二成分(\texttt{status(2)})]
    $\alpha$が発散した場合には\verb|1|が返される.
    \verb|COCG_update|もしくは\verb|BiCG_update|で,
    残差ベクトルと影の残差ベクトルが直交した場合には\verb|2|が返される.
    それ以外の場合には\verb|0|が返される.
    \verb|status(1)|と\verb|status(2)|の両方が\verb|0|の時のみ反復を続行できる.
...
PROGRAM my_prog
  ...
  ELSE
     ...
     !v4(1:ndim) = v2(1:ndim)
     !
     CALL BiCG_init(ndim, nl, nz, x, z, max(0,itermax), &
     &              threshold, status)
     !
  END IF
  !
