...
ふたつの namelist (\verb|filename|および\verb|parameter|)に分かれています。
...
\item \verb|InHam|
  ...
  {\bf 形式 :} string型 (default: \verb|"zvo_Ham.dat"|)
  
  {\bf 説明 :} MatrixMarket形式のHamiltonianファイル名(フルパスもしくは相対パス)を指定します。
  
\item \verb|InVec|
  
  {\bf 形式 :} string型 (default: \verb|"zvo_Excited.dat"|) 

  {\bf 説明 :} 励起ベクトルを格納したファイル名を指定します。

\item  \verb|OmegaMin|
  
  {\bf 形式 :} 複素数 (default: \verb|(0.0, 1.0)|)

  {\bf 説明 :} 振動数の始点を指定します。
  
\item  \verb|OmegaMax|
  
  {\bf 形式 :} 複素数 (default: \verb|(1.0, 1.0)|)
